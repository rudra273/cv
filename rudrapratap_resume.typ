...
#set document(
  ...
  keywords: (
    "MLOps", "DataOps", "LangChain", "LangGraph", "AWS", "Azure",
    "Databricks", "Kubernetes", "Docker", "Generative AI", "RAG",
    "LLM", "Python", "CI/CD", "SageMaker", "MLflow", "FastAPI",
    "Terraform", "Airflow", "Helm", "Agentic AI",
  ),
  description: "MLOps Engineer specializing in end-to-end ML pipelines and agentic AI systems.",
  date: auto,
)

// ─── Page & Typography ───────────────────────────────────────
#set page(
  margin: (x: 0.55in, y: 0.35in),
  paper: "a4",
)
#set text(
  font: "Avenir Next",
...
// ─── Role Entry ──────────────────────────────────────────────
#let role(title, date, body) = {
  place(left, dx: -11.5pt, dy: 1.5pt)[
    #circle(radius: 2.2pt, fill: white, stroke: 1.5pt + clr-secondary.lighten(40%))
  ]
  grid(
    columns: (1fr, auto),
    align: (left, right),
    text(style: "italic", size: 8.4pt, fill: clr-subtle)[#title],
...
// ─── Company Block ────────────────────────────────────────────
#let company(name, body) = {
  text(weight: "bold", size: 9.5pt, fill: clr-primary)[#name]
  v(2pt)
  pad(left: 3pt)[
    #block(
...
// ─── Side Entry ───────────────────────────────────────────────
#let side_entry(title, date, sub: none) = {
  grid(
    columns: (60%, 1fr),
    text(weight: "bold", size: 9pt, fill: clr-primary)[#title],
    align(right, text(fill: clr-subtle, size: 8pt)[#date]),
  )
  if sub != none {
    v(-3.5pt)
    text(style: "italic", size: 8pt, fill: clr-content)[#sub]
  }
  v(1.5pt)
}

// ─── Side Skill ───────────────────────────────────────────────
#let side_skill(cate, items) = {
  text(size: 9pt, fill: clr-side-label)[#cate]
  linebreak()
  text(size: 8.2pt, fill: clr-content)[#items]
  v(0.2pt)
}
...
#company("Sigmoid (Astar Data LLP)")[
      #role("MLOps Engineer  |  Bengaluru, India", "Feb 2024 – Present")[
    ...
    #company("Test Yantra")[
      #role("Associate Software Engineer  |  Bengaluru, India", "Mar 2023 – Feb 2024")[
  ...
  [
    ...
    #side_skill("Programming & Core CS", "Python, JavaScript, SQL, DSA, Linux, Git")
    #side_skill(
      "Agentic AI & LLM Systems",
      "LangChain, LangGraph, Langfuse, RAG, Prompt Engineering, ChromaDB, LLMOps, Azure AI Foundry",
    )
    #side_skill(
      "MLOps & Production ML",
      "MLflow, SageMaker, Ray, ZenML, Databricks, Model Drift Detection, Automated Retraining, Experiment Tracking",
    )
    #side_skill(
      "Cloud & DevOps",
      "AWS, Azure, Docker, Kubernetes, Helm, Terraform, Airflow, GitHub Actions, Azure DevOps, EventBridge, Lambda",
    )
    #side_skill(
      "Web & Databases",
      "Django, FastAPI, Next.js, Tailwind CSS, PostgreSQL, MongoDB, Redis, Celery",
    )
    ...
    #side_entry(
      ...
      "2018 – 2022",
      sub: "B.Tech in CS&E | Rourkela, India",
    )

    #v(7pt)
    #section("Certifications")

    #side_entry("AWS ML Engineer – Associate", "2024")
    #v(-6.5pt)
    #text(size: 7.8pt, fill: clr-subtle, style: "italic")[
      MLA-C01 · Designing, building & deploying *ML solutions on AWS*.
    ]
  ],
)